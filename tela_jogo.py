...
class TelaJogo:
    ...
    def criar_tela_jogo(self):
        ...
        self.root.configure(bg="#AFEEEE")

        container = tk.Frame(self.root, bg="black", padx=3, pady=3)
        container.pack(fill="both", expand=True)

        game_frame = tk.Frame(container, bg="#AFEEEE")
        game_frame.pack(fill="both", expand=True)

        cabecalho = tk.Frame(game_frame, bg="#AFEEEE")
        cabecalho.pack(pady=10)

        tk.Label(cabecalho, text="Pontuação:", bg="#AFEEEE").grid(row=0, column=0, padx=10)
        tk.Label(cabecalho, textvariable=self.pontuacao_var, bg="#AFEEEE").grid(row=0, column=1, padx=10)
        tk.Label(cabecalho, text="Partida:", bg="#AFEEEE").grid(row=0, column=2, padx=10)
        tk.Label(cabecalho, textvariable=self.partida_var, bg="#AFEEEE").grid(row=0, column=3, padx=10)
        tk.Label(cabecalho, text="Tempo:", bg="#AFEEEE").grid(row=0, column=4, padx=10)
        tk.Label(cabecalho, textvariable=self.tempo_var, bg="#AFEEEE").grid(row=0, column=5, padx=10)
        botao_parar = tk.Button(cabecalho, text="Parar", font=("", 10), command=self.pararJogo)
        botao_parar.grid(row=0, column=6, padx=10)

        self.numeros_frame = tk.Frame(game_frame, bg="#AFEEEE")
        self.numeros_frame.pack(pady=40)
        tk.Label(self.numeros_frame, textvariable=self.num1_var, font=("Monaco", 32), bg="#AFEEEE").pack(side="left", padx=20)
        tk.Label(self.numeros_frame, textvariable=self.op_var, font=("Monaco", 32), bg="#AFEEEE").pack(side="left", padx=20)
        tk.Label(self.numeros_frame, textvariable=self.num2_var, font=("Monaco", 32), bg="#AFEEEE").pack(side="left", padx=20)
        tk.Label(self.numeros_frame, text="=", font=("Monaco", 32), bg="#AFEEEE").pack(side="left", padx=20)
        tk.Label(self.numeros_frame, textvariable=self.res_var, font=("Monaco", 32), bg="#AFEEEE").pack(side="left", padx=20)

        operacoes_frame = tk.Frame(game_frame, bg="#AFEEEE")
        ...
        for op_texto, func in operacoes.items():
            tk.Button(operacoes_frame, text=op_texto, font=("Monaco", 16), width=5, height=2,
                      command=lambda o=op_texto, f=func: self.verificar_resposta(o, f)).pack(side="left", padx=10)

        self.mensagem_label = tk.Label(game_frame, text="", font=("Monaco", 14), bg="#AFEEEE")
    ...
    def pararJogo(self):
        if self.timer_id:
            self.root.after_cancel(self.timer_id)
        self.resetaTela()
        fim_jogo_frame = tk.Frame(self.root, bg="#AFEEEE")
        fim_jogo_frame.pack(pady=50)
        tk.Label(fim_jogo_frame, text="Fim de Jogo!", font=("Monaco", 24), bg="#AFEEEE").pack()
        tk.Label(fim_jogo_frame, text=f"Sua pontuação final: {self.pontuacao}", font=("Monaco", 18), bg="#AFEEEE").pack()
